...
- (UIButton *)replyBtnNode
{
    if (!_replyBtnNode) {
        ...
        // 获取图片
        NSBundle *bundle = [NSBundle bundleWithURL: [[NSBundle bundleForClass:[self class]] URLForResource:@"NewsListModule" withExtension:@"bundle"]];
        UIImage *image = [UIImage imageNamed:@"common_chat_new" inBundle:bundle compatibleWithTraitCollection:nil];
        [replyBtnNode setImage:image forState:UIControlStateNormal];
